...
function stripNamespaces(name, options){
  return options.splitNamespaces ? name.split(':').pop() : name
}
...
module.exports = function(xml, _options) {
    ...
    parser.on('startElement', (name, attrs) => { return startElement(stripNamespaces(name, options))})
    parser.on('text', text)
    parser.on('endElement', (name) => { return endElement(stripNamespaces(name, options)) })
    ...
    var schema = {
        ...
        stripNamespaces: joi.boolean().default(false)
    };
